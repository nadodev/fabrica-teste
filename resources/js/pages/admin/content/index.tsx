            ...
            {safeItems.map((item) => (
              <div key={item.id} className="flex items-center gap-4 p-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-display text-lg font-black text-navy">{label(item)}</div>
                  <div className="text-xs text-text-muted">Ordem {item.sort_order ?? 0} - {item.is_active ? "Ativo" : "Inativo"}</div>
                </div>
                <Link href={`/admin/conteudo/${type}/${item.id}/editar`} className="rounded-md p-2 text-navy hover:bg-bg-soft" aria-label="Editar">
                  <Pencil className="h-4 w-4" />
                </Link>
                <button onClick={() => remove(item)} className="rounded-md p-2 text-red-700 hover:bg-red-50" aria-label="Remover">
