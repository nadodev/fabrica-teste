...
export default function CouponForm({ coupon }: { coupon: Coupon | null }) {
    const form = useForm({
        ...
        minimum_amount: coupon ? coupon.minimum_amount / 100 : 0,
    ...
    const submit = (event: FormEvent) => {
        ...
        form.transform((data) => ({
            ...
            minimum_amount: Math.round(Number(data.minimum_amount || 0) * 100),
            usage_limit:
    ...
    return (
        <AdminLayout title={coupon ? 'Editar cupom' : 'Novo cupom'}>
            ...
            <form
                onSubmit={submit}
                className="max-w-3xl space-y-5 rounded-xl border border-border bg-white p-6"
            >
                <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    >
                        <input
                            ...
                            step="0.01"
                            className="input"
                            value={form.data.minimum_amount}
                            onChange={(event) =>
                                form.setData(
                                    'minimum_amount',
                                    Number(event.target.value),
                ...
                <div className="flex justify-end border-t border-border pt-5">
                    ...
                    >
                        ...
                        {form.processing ? 'Salvando...' : 'Salvar cupom'}
                    </button>
                </div>
            </form>
        </AdminLayout>
