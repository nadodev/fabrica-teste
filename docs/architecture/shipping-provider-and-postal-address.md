...
O Melhor Envio continua encapsulado no cliente de infraestrutura existente, usando a URL base oficial de cada ambiente, `Authorization: Bearer` e `User-Agent` com contato. O token e normalizado e criptografado.
...
O frontend nao depende de CORS nem conhece provedores ou credenciais. Uma futura troca do ViaCEP exige somente outro adaptador. A renovacao OAuth do Melhor Envio continua sendo uma responsabilidade operacional separada.
